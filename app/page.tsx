...
export default function InboxPage() {
  ...
  return (
    <div className="flex h-full overflow-hidden" style={{ background: 'var(--bg-page)', color: 'var(--text-primary)' }}>
      {/* Left Sidebar - Conversations List */}
      <div className="w-[320px] flex flex-col h-full" style={{ background: 'var(--bg-card)', borderRight: '1px solid var(--border-main)' }}>
        <div className="p-4" style={{ borderBottom: '1px solid var(--border-main)' }}>
          <h1 className="text-[18px] font-bold mb-3" style={{ color: 'var(--text-primary)' }}>Inbox</h1>
          ...
          {INBOX_CONVERSATIONS.map((conv) => {
            ...
            return (
              <div
                ...
                style={{ borderBottom: '1px solid var(--border-main)', background: isActive ? 'var(--bg-selected)' : undefined }}
              >
                <div>
                  <h3 className="text-[13px] font-semibold leading-tight mb-0.5" style={{ color: 'var(--text-primary)' }}>
                    {conv.name}
                  </h3>
                  <p className={`text-[11px]`} style={{ color: isActive ? '#0066FF' : 'var(--text-secondary)' }}>{conv.agent}</p>
                </div>
                <div className="flex flex-col items-end gap-1.5">
                  <span className={`text-[9px] font-bold tracking-wide px-1.5 py-0.5 rounded`} style={{ background: isActive ? 'var(--bg-card)' : 'var(--bg-pill)', color: isActive ? '#0066FF' : 'var(--text-secondary)' }}>
                    {conv.channel}
                  </span>
                  {conv.unreadCount && (
      ...
      {/* Middle Area - Conversation Detail */}
      <div className="flex-1 flex flex-col min-w-0" style={{ background: 'var(--bg-page)' }}>
        {activeConv ? (
          <>
            {/* Chat Header */}
            <div className="flex items-center justify-between px-6 py-3" style={{ background: 'var(--bg-card)', borderBottom: '1px solid var(--border-main)' }}>
              <div>
                <h2 className="text-[15px] font-bold" style={{ color: 'var(--text-primary)' }}>{activeConv.name}</h2>
                <div className="flex items-center text-[11px] mt-0.5" style={{ color: 'var(--text-secondary)' }}>
                  <span>{activeConv.channel === "WA" ? "WhatsApp" : activeConv.channel === "IG" ? "Instagram" : "Messenger"}</span>
                  {activeConv.channel === "WA" && (
                    <>
                      <span className="mx-1.5">•</span>
                      <span className={activeConv.windowLeft === "Closed" ? "text-[#FF9500]" : "text-[#34C759]"}>
                        {activeConv.windowLeft === "Closed" ? "Reply window closed" : `Window: ${activeConv.windowLeft}`}
                      </span>
                    </>
                  )}
                </div>
                ...
                <div key={msg.id} className={`flex ${msg.isIncoming ? "justify-start" : "justify-end"}`}>
                  <div className={`max-w-[70%] rounded-2xl px-4 py-3 ${
                    msg.isIncoming 
                      ? "rounded-tl-sm" 
                      : "rounded-tr-sm"
                  }`} style={{ background: msg.isIncoming ? 'var(--bg-chat-incoming)' : 'var(--bg-chat-outgoing)', border: msg.isIncoming ? '1px solid var(--border-main)' : 'none', boxShadow: 'var(--shadow-card)' }}>
                    <p className={`text-[15px] leading-relaxed ${
                      msg.dir === "rtl" ? "text-right" : "text-left"
                    }`} style={{ color: msg.isIncoming ? 'var(--text-chat-incoming)' : 'var(--text-chat-outgoing)' }} dir={msg.dir}>
                      {msg.text}
                    </p>
                    <div className={`text-[10px] mt-1.5 ${
                      msg.dir === "rtl" ? "text-right" : "text-left"
                    }`} style={{ color: msg.isIncoming ? 'var(--text-muted)' : 'var(--text-chat-outgoing)', opacity: msg.isIncoming ? 1 : 0.6 }}>
                      {!msg.isIncoming && msg.isRead && "✓✓ "}{msg.time}
                    </div>
                  </div>
                </div>
              ))}
            ...
            {/* Chat Input */}
            <div className="px-6 py-4 shrink-0" style={{ background: 'var(--bg-card)', borderTop: '1px solid var(--border-main)' }}>
              ...
              {activeConv.windowLeft === "Closed" ? (
                <div className="rounded-[8px] p-4 text-[13px] text-center" style={{ background: 'var(--bg-input)', border: '1px solid var(--border-main)', color: 'var(--text-secondary)' }}>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <button className="bg-[#78B4F9] text-white font-semibold text-[13px] px-5 rounded-[8px]">
                    Send
                  </button>
                </div>
              )}
          ...
          <div className="flex-1 flex items-center justify-center">
            <p className="text-[13px]" style={{ color: 'var(--text-secondary)' }}>Select a conversation</p>
          </div>
        )}
      </div>
      ...
      {activeConv && (
        <div className="w-[300px] flex flex-col h-full overflow-y-auto p-5 shrink-0" style={{ background: 'var(--bg-panel)', borderLeft: '1px solid var(--border-main)' }}>
          <div className="mb-6">
            <h2 className="text-[16px] font-bold" style={{ color: 'var(--text-primary)' }}>{activeConv.name}</h2>
            <p className="text-[12px]" style={{ color: 'var(--text-secondary)' }}>{activeConv.phone}</p>
          </div>
          ...
          <div className="mb-5">
            ...
            {activeConv.source === "direct" ? (
              ...
              <div className="rounded-[6px] p-2.5" style={{ background: 'var(--bg-input)' }}>
                <p className="text-[11px] font-bold mb-0.5" style={{ color: 'var(--text-primary)' }}>{activeConv.source.campaign}</p>
                <p className="text-[10px]" style={{ color: 'var(--text-secondary)' }}>{activeConv.source.adset}</p>
                <p className="text-[10px]" style={{ color: 'var(--text-secondary)' }}>{activeConv.source.ad}</p>
              </div>
            )}
          </div>
          ...
          <div className="mb-5">
            ...
            {activeConv.pipeline === "direct" ? (
              <p className="text-[11px]" style={{ color: 'var(--text-secondary)' }}>No lead record — this contact messaged directly.</p>
            ) : (
              <select 
          ...
          <div className="mb-5">
            ...
            <input type="text" placeholder="e.g. Call back about Sunday" className="w-full rounded-[6px] text-[11px] py-1.5 px-2 mb-2 outline-none placeholder-gray-400" style={{ border: '1px solid var(--border-input)', background: 'var(--bg-input)', color: 'var(--text-primary)' }} />
            <div className="flex gap-2">
              <div className="flex-1 rounded-[6px] px-2 py-1.5 flex items-center justify-between" style={{ border: '1px solid var(--border-input)', background: 'var(--bg-input)' }}>
                <span className="text-[11px]" style={{ color: 'var(--text-primary)' }}>09 / 21 / 2026</span>
                <svg className="w-3 h-3" style={{ color: 'var(--text-secondary)' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
              </div>
